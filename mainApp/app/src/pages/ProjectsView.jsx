    ...
    <div className="project-view-container">
      ...
      {activeTab === "overview" && project && (
          <div className="project-overview-widget-container">
            <div className="project-overview-widget">
              {isEditing ? (
                ...
                <span style={{"color": "red"}}>{errorMessage}</span>
                  ...
                  <input type="datetime-local" value={editProject.deadline} onChange={(e) => setEditProject({ ...editProject, deadline: e.target.value })} />
                  <Select isMulti options={users} value={selectedMembers} onChange={setSelectedMembers} placeholder={t("choose_assigned_users")} />
                  {console.log(editProject)}
                  <Select options={projectStatuses} value={projectStatuses.find(status => status.label === editProject.status)} onChange={(option) => setEditProject({ ...editProject, status: option.label })} />
                  <label>
                  ...
                  <button onClick={updateProjectDetails}>{t("save")}</button>
                  {isEditing ? null : <button onClick={startEditing}>{t("edit")}</button>}
                  {!isEditing ? null : <button onClick={stopEditing}>{t("cancel")}</button>}
                </>
              ) : (
                <>
                  <h2>{project.title}</h2>
                  <p>{project.description}</p>
                  <p><strong>{t("status")}:</strong> {projectStatuses.find(status => status.label === project?.status)?.label || t("unknown")}</p>
                  <p><strong>{t("deadline")}:</strong> {new Date(project.deadline).toLocaleString()}</p>
                  <p><strong>{t("is_servicedesk")}:</strong> {project.isServiceDesk ? "☑️" : "❌"}</p>
                  {isEditing ? null : <button onClick={startEditing}>{t("edit")}</button>}
                  {!isEditing ? null : <button onClick={stopEditing}>{t("cancel")}</button>}
                </>
              )}
            ...
            <div className="project-overview-widget">
              <h2>{t("open_tasks")}</h2>
              <h2>{tasks.filter(task => !task.isDone).length}</h2>
